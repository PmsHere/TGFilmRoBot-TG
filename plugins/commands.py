...
from info import (ADMINS, AUTH_CHANNEL, BATCH_FILE_CAPTION, CHANNELS,
                  CUSTOM_FILE_CAPTION, LOG_CHANNEL, PICS, PROTECT_CONTENT)
from Script import script
from utils import (get_settings, get_size, getInviteLink, is_subscribed,
...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "💢 𝙹𝚘𝚒𝚗 𝙾𝚞𝚛 𝙲𝚑𝚊𝚗𝚗𝚎𝚕 💢", url=invite_link.invite_link
                )
            ]
        ]
...
def get_link(client, message):
    ...
    try:
        ...
        if invite_link:
            message.reply_text(f"Invite link for chat {chat.title}: {invite_link}")
        else:
        ...
        admin_list = "\n".join([f"{admin.user.first_name} (@{admin.user.username}) [{admin.user.id}]" for admin in admins])
        message.reply_text(f"Administrators in chat {chat.title}:\n{admin_list}")
